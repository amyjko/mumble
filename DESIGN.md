...
- [ ] **UX-AV-4** (MVP) - Transient emotes include 1) emoji reactions that float above participants' A/V object, 2) a bounce animation to indicate excitement, 3) a "laying down" animation to indicate boredom, 4) a "coin spin" animation to indicate excitement. Transient emotes broadcast to everyone, animate, and vanish — never stored.
