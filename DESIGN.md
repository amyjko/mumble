...
- **Keep these current as you work.** They were all left unchecked for the first ~30 commits, which made the document say nothing was done when a third of it was. Status audited against the code 2026-07-18: **63 done · 33 partial · 56 blocked · 15 not started** (167 total).
...
- [ ] **UX-PERM-3** (MVP) — Hosts may edit room configurations, admit participants, set creation permissions, and edit/delete `host`-permission objects. Host role is room state, typically the account holder who owns the room. _BLOCKED: needs auth and the host role._
...
- [ ] **UX-STAGE-4** (MVP) — The queue is FIFO. Releasing a slot passes it to the head of the queue: muting releases your audio slot (UX-STAGE-10), turning the camera off releases your video slot, and leaving releases both. A host may grant, revoke, or preempt any slot directly, which is the escape hatch when the queue needs overriding. _PARTIAL: FIFO and handoff done and tested; host grant/revoke exists but is unreachable and ungated until the host role lands._
...
- [ ] **UX-STAGE-11** (MVP) — Admission is refused once the room holds `max_participants`; guests awaiting admission (UX-ID-3) do not count against it until admitted. _PARTIAL: full-room refusal done and tested; "guests awaiting admission" is vacuous until there is a pending state._
...
- [ ] **UX-ID-4** (MVP) — Hosts hold accounts; guests need not. Anonymous guests accrue usage against the host's/room's limits per room policy. _BLOCKED: needs auth and the host role._
...
- [ ] **UX-ID-6** (MVP) — Logging in carries identity and avatars across machines — the same person on laptop and phone is one identity with one set of owned objects. _BLOCKED: needs auth and the host role._
- [ ] **UX-ID-7** (MVP; passkeys Later) — Login offers diverse low-friction options suited to both laptops and phones: email magic link and OAuth at minimum; passkeys when the provider's support is production-ready (AR-AUTH-5). _BLOCKED: needs auth and the host role._
...
- [ ] **UX-ID-9** (MVP) — Avatar data (including the camera-off emoji) works without login (per-browser) and roams with login. _PARTIAL: works without login; roaming with login needs auth._
...
- [ ] **UX-ROOM-6** (MVP) — Hosts may edit rooms and configurations at any time, including live during a meeting, synchronized to all participants. _PARTIAL: live synchronized editing is done and tested; the "hosts may" gate is deliberately inert._
...
- [ ] **AR-BACKEND-1** (MVP) — Supabase provides the four backend surfaces: Postgres (objects, rooms, configurations, ledger), Realtime (sync channels), Auth (all identity), Storage (image objects). — _serves: UX-OBJ-10, UX-OBJ-5, UX-ID-1..9, UX-ECON-2._ _PARTIAL: only Auth is scaffolded; no Postgres schema, Realtime, or Storage._
...
- [ ] **AR-BACKEND-9** (MVP) — TypeScript types are generated from the Postgres schema (`supabase gen types typescript`) and used end-to-end; local dev runs the full Supabase stack via the CLI. **[NO-UX-LINK — developer experience.]** _PARTIAL: the generation script and config exist; the generated Database type has no tables and CI never starts Supabase._
- [ ] **AR-BACKEND-10** (MVP) — Room names are enforced in the schema, not just in application code: `rooms.name` is `citext` with a unique index (giving case-insensitive uniqueness for free) plus a `CHECK` constraint for the charset and length rule; reserved names live in a table the same constraint path consults. The room route is `src/routes/hey/[room]/+page.svelte`; a name that does not resolve 404s. — _serves: UX-ROOM-8..10._ _PARTIAL: the 404 route is done and tested; citext, the unique index, and the reserved-names table do not exist._

## 15. Authentication and identity (AR-AUTH)

- [ ] **AR-AUTH-1** (MVP) — Supabase Auth is the single identity system. Anonymous guests use `signInAnonymously()` — replacing the previously-planned hand-rolled local-storage identity token — so `creator_id` = the auth user id for guests and account holders alike, one code path. — _serves: UX-ID-1, UX-ID-5._ _BLOCKED: needs auth and the host role._
- [ ] **AR-AUTH-2** (MVP) — Anonymous vs. permanent distinctions use the `is_anonymous` JWT claim, and policies that restrict anonymous users are written as **RESTRICTIVE** RLS policies (permissive policies OR together — a documented footgun). — _serves: UX-ID-4, UX-PERM-1._ _BLOCKED: needs auth and the host role._
...
- [ ] **AR-AUTH-4** (MVP) — Host login: magic link + OAuth through `@supabase/ssr` cookie sessions (server client created in `hooks.server.ts`; session available across load functions, layouts, and server routes). Route guards and permission enforcement are ours to write — the package wires clients, it does not enforce. — _serves: UX-ID-6, UX-ID-7, UX-PERM-3._ _PARTIAL: the SSR client and memoized claims are real; no login routes, guards, or UI._
...
- [ ] **AR-AUTH-6** (MVP) — Accepted limitation, surfaced in UX copy: an anonymous identity is browser/device-bound (lost on sign-out, cleared storage, or another device); cross-device continuity requires upgrading to an account first. — _serves: UX-ID-5, UX-ID-6 (defines their boundary)._ _PARTIAL: the limitation is true and documented in code, but never surfaced in UX copy._
...
- [ ] **AR-CANVAS-3** (MVP) — One polymorphic schema for all objects; type-specific data in `payload`: _PARTIAL: the polymorphic schema is implemented EXCEPT `anchor` — which UX-OBJ-12 and AR-CANVAS-5 both depend on, so that exemption can never fire — plus the av/screenshare/image variants._
...
- [ ] **AR-TEST-2** (MVP) — The whole stack runs offline against the Supabase CLI (`supabase start`) with no network connection after the first image pull: Postgres, Auth, Realtime, Storage, and a local SMTP catcher. **Supabase CLI ≥ 2.108.0 is a hard floor** — earlier versions fetch a remote Deno import on every start and simply do not work offline. **[NO-UX-LINK — developer experience.]** _PARTIAL: config.toml exists; no migrations and no Supabase in CI._
...
- **`anchor` is specified but does not exist.** AR-CANVAS-3 declares `anchor? { target_id, offset, scale_with_target }`, and BOTH UX-OBJ-12 and AR-CANVAS-5 exempt anchored objects from the overlap solver — but the field is absent from `schemas.ts`, so that exemption can never fire and drawings are the only objects that actually skip the solver. This is the same shape as the old `permission` gap: fully specified, reasoned about in three places, unreachable in code. Decide deliberately — implement anchoring, or strike it the way arbitrary path clips were struck (UX-OBJ-7) — rather than leaving requirement text that describes behavior the product does not have.
