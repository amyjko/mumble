...
- [ ] **AR-CTRL-5** (MVP) — The admission flow (pending state, hello message, pre-admission chat, admit/decline) is control-plane state with its own Broadcast channel per waiting guest (AR-BACKEND-6). Admission also enforces the `max_participants` gate (UX-STAGE-11). — _serves: UX-ID-2..3, UX-STAGE-11._ _NOT STARTED: not blocked any more — the control plane exists. Unbuilt: the pending state and its per-guest Broadcast channel._
