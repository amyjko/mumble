-- Per-object versions (AR-BACKEND-4).
--
-- A conflict should mean "someone else edited THIS object". Until now it meant
-- "someone else edited this ROOM", which is both too loud and too quiet:
--
--   Too loud — `edit_note` was guarded room-wide, so two people typing in two
--   DIFFERENT notes conflicted continuously. Each conflict costs a re-read and
--   a re-apply, and once the bounded retry is exhausted the edit is dropped.
--
--   Too quiet — an object write that changed no room scalar was UNGUARDED, so
--   two people dragging the SAME object silently last-writer-wins.
--
-- The room counter stays. It is what the Realtime broadcast carries and what
-- the client compares to drop its own echo, and it is still the right guard for
-- room-scalar writes and for `upsert_participant`, which decides against a cap
-- by counting rows it does not write (UX-STAGE-11) and so cannot be expressed
-- per row.
--
-- Versions are deliberately NOT part of `RoomState`. They ride in the RPC
-- envelope as a side map, because `model/diff.ts` compares rows structurally to
-- decide what changed — a version inside the row would read as content, every
-- object would compare unequal, and the whole-room write that diff.ts exists to
-- eliminate would come straight back.
--
-- What this does NOT fix: overlap. Two writers moving two DIFFERENT objects
-- into the same space each pass their own row's check, so UX-OBJ-12 remains
-- violable under concurrency and stays PARTIAL in DESIGN.md.

alter table room_objects add column if not exists version bigint not null default 0;

-- Read the versions alongside the state, in the SAME consistent snapshot.
