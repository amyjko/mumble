-- Screen shares consume a video slot (UX-OBJ-6, UX-STAGE-1).
--
-- A third holder list rather than a second entry in `video_holders`: that array
-- is a set of participant ids, so one person sharing a screen AND a camera
-- cannot appear in it twice. `max_av` now bounds the SUM of the two lists —
-- publishing both costs you two of the room's video capacity, which is what the
-- requirement asks for and what the bandwidth actually is.
--
-- The two state functions are re-declared in full because Postgres has no way
-- to add a key to a `jsonb_build_object` in place. Both halves matter: without
-- the `save_room_state` half, screen slots round-trip as always-empty and the
-- failure presents as a broken publish path rather than a missing column.

alter table room_state add column if not exists screen_holders uuid[] not null default '{}';
...
as $$
	select jsonb_build_object(
		'version', r.version,
		-- The GEOMETRY counter, bumped only by writes that decide against other
		-- shapes. Separate from `version` so that typing does not invalidate a
		-- drag: every write bumps `version`, and CASing geometry on it would
		-- make a drag conflict with every keystroke in the room.
		...
		'state', jsonb_build_object(
			...
			-- Drawn from the same `max_av` pool as `video_holders`; see stage.ts.
			'screen_holders', to_jsonb(s.screen_holders),
...
create or replace function public.save_room_state(
	...
	p_expected_geometry bigint default null
)
returns bigint
language plpgsql
security definer
...
begin
	...
	/*
	 * The GEOMETRY compare-and-swap (UX-OBJ-12).
	 *
	 * Overlap is a CROSS-ROW invariant: `move_object` asks whether its new
	 * position collides with every other shape in the room. Two writers moving
	 * two DIFFERENT objects each pass their own row's check against a snapshot
	 * that does not contain the other, and land an overlap neither would have
	 * been allowed alone. Per-object versions cannot see this — the rows do not
	 * collide, the SHAPES do.
	 *
	 * So geometry writes serialise against each other, and only against each
	 * other. The loser re-reads and re-applies, and the overlap check then sees
	 * the winner's shape and refuses properly (UX-PERM-4's visible revert).
	 *
	 * Bumped ONLY on a geometry write, which is the point of a second counter:
	 * a keystroke must not invalidate someone else's drag.
	 */
	if p_expected_geometry is not null then
		update rooms set geometry_version = geometry_version + 1
		where id = p_room_id and geometry_version = p_expected_geometry
		returning geometry_version into next_geometry;

		if next_geometry is null then
			raise exception 'geometry conflict' using errcode = 'PT409';
		end if;
	end if;
	...
	if room_scalars is not null and jsonb_typeof(room_scalars) = 'object' then
		update room_state set
			...
			screen_holders = coalesce((select array_agg(value::uuid) from jsonb_array_elements_text(room_scalars->'screen_holders')), '{}'),
...
revoke all on function public.save_room_state(uuid, jsonb, bigint, jsonb, text, bigint) from public;
grant execute on function public.save_room_state(uuid, jsonb, bigint, jsonb, text, bigint) to service_role;
