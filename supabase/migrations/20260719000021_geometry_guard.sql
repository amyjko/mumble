-- Serialise geometry writes, so overlap cannot be produced by two writers
-- (UX-OBJ-12).
--
-- Every other guard in this system is per-ROW: the room counter for scalars,
-- per-object versions for object content. Overlap fits neither, because it is a
-- cross-row invariant — `move_object` validates its new position against every
-- other shape in the room. Two people dragging two DIFFERENT objects into the
-- same space each pass their own check against a snapshot that lacks the other,
-- and the room ends up in a state the rule engine would have refused either
-- writer alone.
--
-- A SECOND counter rather than reusing `rooms.version`: every write bumps that
-- one, so a drag CASing on it would conflict with every keystroke in the room —
-- which is the room-wide false conflict that per-object versions were added to
-- remove. This one is bumped only by writes that decide against other shapes.

alter table rooms add column if not exists geometry_version bigint not null default 0;
...
drop function if exists public.save_room_state(uuid, jsonb, bigint, jsonb, text, bigint);
